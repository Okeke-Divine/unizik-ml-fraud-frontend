...
export default function AdminAppealsPage() {
  ...
  const handleAction = async (appealId: string, status: "APPROVED" | "REJECTED") => {
    ...
    try {
      ...
      if (res.ok) {
        fetchAppeals();
        setExpandedId(null); // Close drawer on complete
      }
    } finally {
                  ...
                  {filteredAppeals.map((item) => {
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        ...
                        {/* Expandable Forensic Drawer (Clean Institutional Light Mode) */}
                        {isExpanded && (
                          <tr className="bg-slate-50/80 border-b border-slate-200 shadow-inner animate-in fade-in duration-150">
                            <td colSpan={5} className="p-6">
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                
                                {/* Student Remediation Request (Slate Theme) */}
                                <div className="bg-white border border-slate-200/80 p-5 rounded-xl shadow-2xs space-y-2.5">
                                ...
                                {/* White-Box XAI Telemetry Log (Soft Rose Theme) */}
